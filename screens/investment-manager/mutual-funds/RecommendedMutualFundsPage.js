...
const calculateScore = (fund) => {
  const returnScore =
    (parseFloat(fund["3Y Returns"]) + parseFloat(fund["5Y Returns"])) / 2;
  const expenseScore = 1 / parseFloat(fund["Expense Ratio"].replace("%", "")); // Lower expense ratio, higher score
  const ratingScore = parseFloat(fund["Rating"]) * 10; // Scaling rating to make it comparable

  // Weighting the factors: you can adjust the weights based on your preference
  const score = returnScore * 0.5 + expenseScore * 0.3 + ratingScore * 0.2;
  return score;
};

const RecommendedMutualFundsPage = () => {
  ...
  useEffect(() => {
    // Calculate scores and sort mutual funds
    const scoredFunds = mutualFundsData.map((fund) => ({
      ...fund,
      score: calculateScore(fund),
    }));
    scoredFunds.sort((a, b) => b.score - a.score); // Sort by score descending

    // Select top 5 recommended funds
    setRecommendedFunds(scoredFunds.slice(0, 5));
    setLoading(false);
    fetchDetailsStatus();
  }, []);
        ...
        {loading ? (
          ...
          <ScrollView style={styles.scrollView}>
            {recommendedFunds.length > 0 ? (
              recommendedFunds.map(renderMutualFundCard)
            ) : (
...
export default RecommendedMutualFundsPage;
